...
When(/^I get all the configurations$/) do
  @all_configurations = KalibroClient::Entities::Configurations::Configuration.all
end
...
Then(/^the configuration should exist$/) do
  expect(KalibroClient::Entities::Configurations::Configuration.exists?(@configuration.id)).to be_truthy
end
